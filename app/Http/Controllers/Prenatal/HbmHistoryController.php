...
class HbmHistoryController extends Controller
{
    ...
    public function save(Request $request, PatientsModel $patient)
    {
        ...
        if (!empty($followUp)) {
            try {
                $followUp = Carbon::parse($followUp)->format('Y-m-d');
            } catch (\Throwable $e) {
                $followUp = null;
            }
        }
        ...
        try {
            ...
            if ($isDraft) {
                if (Schema::hasColumn('patients', 'prenatal_history')) {
                    ...
                    $patient->prenatal_history = array_replace($legacy, $data, [
                        '__pregnancy_id' => (int) $pregnancy->id,
            ...
            Activity::record('prenatal.hbm_history.updated', [
                ...
                'properties' => [
                    'pregnancy_id' => (int) $pregnancy->id,
                    'pregnancy_no' => (int) $pregnancy->pregnancy_no,
                ],
            ]);
            ...
            return response()->json([
                'history' => array_merge($model->toArray(), [
                    'pregnancy_id' => (int) $pregnancy->id,
                ]),
            ], 200);
        } catch (\Throwable $e) {
